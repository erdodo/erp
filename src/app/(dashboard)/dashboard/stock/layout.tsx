...
const NAV = [
  { href: "/dashboard/stock",            label: "Stok Listesi", icon: "pi-box" },
  { href: "/dashboard/stock/warehouses", label: "Depolar",      icon: "pi-building" },
];
  ...
  return (
    <div className="max-w-screen-2xl mx-auto space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="font-bold text-foreground text-lg leading-tight">Stok Yönetimi</h1>
          <p className="text-xs text-slate-400">Depo bazlı stok takibi & hareketler</p>
        </div>
      </div>
      <nav className="flex items-center gap-1 border-b border-border">
        {NAV.map((item) => {
          const active = item.href === "/dashboard/stock" ? pathname === item.href : pathname.startsWith(item.href);
          return (

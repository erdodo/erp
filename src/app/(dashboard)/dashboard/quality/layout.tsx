...
const NAV = [
  { href: "/dashboard/quality",           label: "Kalite Kontrol", icon: "pi-verified"  },
  { href: "/dashboard/quality/standards", label: "Standartlar",    icon: "pi-list-check" },
];
    ...
    <div className="max-w-screen-2xl mx-auto space-y-4">
      <div className="flex items-center gap-3">
        <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: "var(--color-primary)" }}>
          <i className="pi pi-verified text-white text-sm" />
        </div>
        <div>
          <h1 className="font-bold text-foreground text-lg leading-tight">Kalite PPM</h1>
          <p className="text-xs text-slate-400">Kontrol kayıtları, PPM analizi, standartlar</p>
        </div>
      </div>
      <nav className="flex items-center gap-1 border-b border-border">
        {NAV.map((item) => {
          const active = item.href === "/dashboard/quality" ? pathname === item.href : pathname.startsWith(item.href);
          return (
